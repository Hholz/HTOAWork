...
<script type="text/javascript">
	...
	function clear() {
		//清空新增div中的数据
		$("#window_flowHoliday #stratTime").val('');
		$("#window_flowHoliday #endTime").val('');
	...
	function saveHoliday() {
		...
		var dateS = new Date(dateS1[0] - 1, dateS1[1] - 1, dateS1[2] + 1,
				dateS2[0], dateS2[1], dateS2[2]);//拼接成date类型
		var dateE = new Date(dateE1[0] - 1, dateE1[1] - 1, dateE1[2] + 1,
				dateE2[0], dateE2[1], dateE2[2]);//拼接成date类型
		var dateST = dateS.getTime();
		var dateET = dateE.getTime();
		if (dateST - dateET > 0) {
			parent.layer.alert('开始请假时间不能超过结束时间');
			return;
		} else if (dateST - dateET == 0) {
			parent.layer.alert('开始请假时间和结束时间一样');
			return;
		}
		var url = "${pageContext.request.contextPath }/flow/flowHolidayEmp/saveFlowHolidayEmp";
		$.post(url, {
	...
	function addHoliday() {
		...
		var url = "${pageContext.request.contextPath }/flow/flowHolidayEmp/addFlowHolidayEmp";
		$.post(url, {
			flowmodeltypeid : $("#window_flowHoliday #flowModelTypeid").val(),
			strattime : $("#window_flowHoliday #stratTime").val(),
			endtime : $("#window_flowHoliday #endTime").val(),
