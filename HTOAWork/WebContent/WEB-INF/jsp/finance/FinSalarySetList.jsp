...
<%@page import="com.ht.popj.sysSet.FinanceSalarytypese"%>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btn_edit").click(function() {
			...
			$("#window_add #empid").val(rows.empid);
			$("#window_add #1").val(rows.jbmoney);
			$("#window_add #2").val(rows.gwmoney);
			$("#window_add #3").val(rows.sbmoney);
			$("#window_add #4").val(rows.ybmoney);
			$("#window_add #5").val(rows.cbmoney);
			$("#window_add #7").val(rows.zfmoney);
			$("#window_add #empname").val(rows.empname);
			$("#window_add #depid").attr("disabled", true);
			$("#window_add #empid").attr("disabled", true);
			selectemp();
		...
		$("#btn_add").click(function() {
			$("#window_add #id").val('');
			$("#window_add #depid").attr("disabled", false);
			$("#window_add #empid").attr("disabled", false);
		...
		checkbox.on("click", function() {
			...
			$("#selectAll").prop('checked',
					_tmp.length == _tmp.filter(":checked").length);
		});
	});
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments')
					.bootstrapTable(
							{
								url : '${pageContext.request.contextPath}/finance/FinanceSalarySet/0', //请求后台的URL（*）
								method : 'post', //请求方式（*）
	...
	function addStudent() {
		...
		obj = $("#window_add #typelist input[type=number]");
		var listJson = "";
		listJson = listJson + "{'stypeid':[";
		for (i = 0; i < obj.length; i++) {
			if (obj[i].value == null || obj[i].value == '') {
				return;
			}
			if (i != obj.length - 1) {
				listJson = listJson + "{'id':" + obj[i].id + "," + "'value':"
						+ obj[i].value + "},";
			} else {
				listJson = listJson + "{'id':" + obj[i].id + "," + "'value':"
						+ obj[i].value + "}";
	...
	function updateStudent() {
		...
		listJson = listJson + "]}";
		var url = "${pageContext.request.contextPath }/finance/FinanceSalarySet/0";
		$.post(url, {
			_method : 'PUT',//改成PUT提交
			empid : $("#window_add #empid").val(),
			scoredListJson : listJson,
		...
		$("#window_add").modal('hide');
		$("#window_add #empid").val('');
		$("#window_add #jbmoney").val('');
		$("#window_add #gwmoney").val('');
		$("#window_add #sbmoney").val('');
		$("#window_add #ybmoney").val('');
		$("#window_add #cbmoney").val('');
		$("#window_add #zfmoney").val('');
	}
					...
					<div class="ibox-content">
						<form id="formSearch" class="form-horizontal">
							<div class="form-group">
								...
								<div class="col-sm-2">
									<select id="dep" class="form-control" onchange="selectdep();">
										<option value="">---------</option>
			...
			<div class="modal-content animated bounceInRight">
				...
				<div class="ibox-content">
					<form class="form-horizontal m-t" id="commentForm"
						...
						<div class="form-group">
							<div class="col-sm-12" id="typelist">

								<%
									List<FinanceSalarytypese> flist = (List) request.getAttribute("fintypelist");
									for (int i = 0; i < flist.size(); i++) {
										FinanceSalarytypese f = flist.get(i);
										if (i % 2 == 0) {
								%>
								<div class="form-group">
									<label class="col-sm-2 control-label"><%=f.getName()%>:</label>
									<div class="col-sm-4" id="typelist">
										<input type="number" required value="<%=f.getSalary()%>"
											id="<%=f.getId()%>" class="form-control" />
									</div>
									<%
										} else {
									%>
