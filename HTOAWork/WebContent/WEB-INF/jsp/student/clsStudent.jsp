...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/Export/tableExport.js"></script>
    ...
    //全局变量，用来保存选中行的数据
		...
		//新增按钮事件*************************
		function add(){
			//因为是在一个班级的情况下做新增，设置学号，设置班级
			//通过后台查询该班级下一个学号，放到页面上
			var url = "${pageContext.request.contextPath }/student/getStunoByClsId";
			$.get(
				url,
				{
					clsId:${cls.id}
	            },
				function(data){//返回的是String,学号
	            	$("#addStudent #stuno").val(data);
	            },
				"text"
			);	
            ...
            $("#addStudent #sex").val('');
            $("#addStudent #age").val('');
            $("#addStudent #classid").val(${cls.id});//设置班级id
            $("#addStudent #middleschool").val('');
            $("#addStudent #birthday").val('');
            $("#addStudent #phone").val('');
            $("#addStudent #addr").val('');
            $("#addStudent #stuStatus").val('1');//默认1，在读状态
            ...
            $("#addStudent #prolevel").val(${cls.majorId});//专业
            $('#addStudent').modal('show');
            //修改按钮的点击事件
		...
		function changeCls2(){
			//弹窗前的清空操作
			$("#changeCls #oldStuno").val('');
			$("#changeCls #newStuno").text('');
			$("#changeCls #stuId").val(''); 
			$("#stuInfo").text('');
			var url = "${pageContext.request.contextPath }/student/getStunoByClsId";
			$.get(
				...
				function(data){//返回的是String,学号
	            	$("#changeCls #newStuno").append(data);
	            },
				"text"
			);	
			$('#window_changeCls').modal('show');
		}
		//通过学生no来查
		function findStuByNo(){
			$("#changeCls #stuId").val(''); 
			$("#stuInfo").text('');//先清空div里的内容
			$("#stuInfo").append("<label class='col-sm-7 control-label'>未查到该学号信息</label>");
			var url = "${pageContext.request.contextPath }/student/findStuByNo";
			$.get(
				url,
				{
					stuno:$('#changeCls #oldStuno').val()
	            },
				function(data){//返回的是String,学号
					if(data){
						var html = "<label class='col-sm-5 control-label'>姓名："+data.stuname+"</label>";
					 	html = html +"<label class='col-sm-5 control-label'>班级："+data.classInfo.classname+"</label>";
					 	$("#stuInfo").text('');//先清空div里的内容
					 	$("#stuInfo").append(html);
					 	$("#changeCls #stuId").val(data.id); //设置要修改的学号
					}
	            },
				"json"
			);	
		...
		//换班,保存
		function change(){
			var stuno = $('#changeCls #oldStuno').val()
			if(stuno==null||stuno==''){
				parent.layer.alert('请输入原学生编号');
				return;
			}
			var id =$("#changeCls #stuId").val(); 
			if(id==null||id==''){
				parent.layer.alert('该编号学生不存在');
				return;
			}
		...
		function change2(id){
			...
			$.post(
				...
				{
					_method:'PUT',//改成PUT提交
					stuno:$('#changeCls #newStuno').text(),
	                classid:${cls.id},
	                oldStuno:$('#changeCls #oldStuno').val(),
	                prolevel:${cls.majorId}
				},
				function(data){
					if(data>0){
						parent.layer.alert('换班成功');
					}else{
						parent.layer.alert('未知错误，换班失败');
					}
				},
				"text"
			);	
			//用来关闭换班窗口***********
			$("#window_changeCls").modal('hide');
		}
		//新增学生，ajax提交
		function addStudent(){
			//用来判断表单是否验证通过
...
<body class="gray-bg">
	<div class="panel-body" style="padding-bottom: 0px;">
						...
						<form id="formSearch" class="form-horizontal">
							<div class="form-group">
								<div class="row">
									...
									<div class="col-sm-1">
										<select class="form-control" name="txt_search_sex" id="txt_search_sex" onchange="clsChange()" class="form-control">
		                                     <option value="">----</option>
								...
								<div class="row">
									...
									<div class="col-sm-1">
										<a href="${pageContext.request.contextPath}/student/myclass/stuList/${clsId }"><button type="button" style="margin-left: 20px" id="btn_query" class="btn btn-primary">清空</button></a>
									</div>
								</div>
							</div>
						</form>
				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<div id="toolbar" class="btn-group">
							...
							<a href="${pageContext.request.contextPath}/student/myclass/page">
							...
							<div class="btn-group dropup">
								...
								<ul class="dropdown-menu" role="menu">
									...
									<li><a onclick='changeCls2()'>换班</a></li>
								</ul>
							</div>
		...
		<div class="modal inmodal" id="window_changeCls" tabindex="-1" role="dialog"
			aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content animated bounceInRight">
					<div class="modal-header">
						...
						<h4 class="modal-title">学生换班</h4>
					</div>
					<div class="ibox-content">
						<form class="form-horizontal m-t"
							novalidate="novalidate" id="changeCls" name="changeCls">
							<div class="form-group">
								<input id="stuId" type="hidden""> 
								<label for="inputPassword" class="col-sm-2 control-label">原学生编号</label>
								<div class="col-sm-4">
									<input type="text" class="form-control" id="oldStuno"  placeholder="请输入学生编号">
								</div>
								<label class="col-sm-2 control-label">新学生编号</label>
								<div class="col-sm-3">
									<p class="form-control-static" id="newStuno"></p>
								</div>
							</div>
							<div class="modal-footer" id="stuInfo">
								
							</div>
							<div class="modal-footer">
								<button type="button" onclick="findStuByNo()" class="btn btn-success">查询学生</button>
								<button type="button" class="btn btn-white" data-dismiss="modal">关闭</button>
