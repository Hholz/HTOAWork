...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btn_edit").click(
				function() {
					...
					$("#window_update #flowmodelname").val(rows.flowmodelname);
					$("#window_update #typename").val(rows.typename);
					$("#window_update #primarystep").val(rows.primarystep);
					$("#window_update #planday").val(rows.planday);
					$("#window_update #seq").val(rows.seq);
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : '${pageContext.request.contextPath}/sysSet/FlowModel/0', //请求后台的URL（*）
				method : 'post', //请求方式（*）
		...
		oTableInit.queryParams = function(params) {
			var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
				...
				flowmodelname : $("#mname").val(),
				flowmodeltypeid : $("#typeid").val(),
				invalid : $("#invalid_id").val(),
			};
			return temp;
		};
		return oTableInit;
	...
	function addStudent() {
		...
		var url = "${pageContext.request.contextPath }/sysSet/FlowModel/add";
		$.post(url, {
			flowmodelname : $("#window_update #flowmodelname").val(),
			flowmodeltypeid : $("#window_update #flowmodeltypeid").val(),
			primarystep : $("#window_update #primarystep").val(),
			planday : $("#window_update #planday").val(),
			seq : $("#window_update #seq").val(),
		...
		$('#tb_departments').bootstrapTable('refresh');
		//清空新增div中的数据
		$("#window_update #flowmodelname").val('');
		$("#window_update #primarystep").val('');
		$("#window_update #seq").val('');
		$("#window_update #planday").val('');
	}
	...
	function updateStudent() {
		...
		var url = "${pageContext.request.contextPath }/sysSet/FlowModel/" + id;
		$.post(url, {
			_method : 'PUT',//改成PUT提交
	...
	<div class="panel-body" style="padding-bottom: 0px;">
					...
					<div class="modal-dialog">
						<div class="modal-content animated bounceInRight">
							...
							<form class="form-horizontal m-t" id="commentForm"
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<div class="col-md-12">
											<div class="form-group">
												<div class="col-sm-9">
													<select id="flowmodeltypeid" class="form-control">
														<c:forEach items="${flowmodeltypeList}" var="cla">
															<option value="${cla.id}">${cla.flowmodeltype}</option>
														</c:forEach>
													</select>
								...
								<div class="form-group">
									<label class="col-sm-3 control-label">关键步骤：</label>
									<div class="col-sm-8">
											<select id="primarystep" class="form-control"
														name="primarystep">
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input id="seq" type="text" required name="seq" class="form-control">
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-3 control-label">计划完成时间：</label>
									<div class="col-sm-8">
										<input id="planday" type="number" required name="planday"
											class="form-control">
									</div>
								</div>
								<div class="form-group">
		...
		function validateForm(formdata){
	        ...
	        var validator = formdata.validate({
	            ...
	            messages: {
	            	flowmodelname: icon + "请输入流程名称",
	            	seq: {
	                	digits: icon + "步骤必须是整数"
                    },
                    planday: {
                    	required:icon + "必须输入，小于1天填1,多于1天不足2天,填2",
                    },
	            },
	        });
