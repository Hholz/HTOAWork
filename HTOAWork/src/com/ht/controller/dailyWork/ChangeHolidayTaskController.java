 ...
 */
@Controller
@RequestMapping("/dailyWork/ChangeHolidayTask")
public class ChangeHolidayTaskController {
	...
	@SystemControllerLog(description = "进入调休审批页面")
	public String List(Model model) {
				...
				if (apmlist.get(j).getEmpid().equals(e.getId())) {
					...
					for (int i = 0; i < apdotlist.size(); i++) {
						...
						if (apdot.getFlowid() == fsch.getFlowdot()) {
							// 若审批节点为 ---领导审批员工调休
							if(apdot.getFlowid() ==26){// 若审批节点为 ---财务部审批员工请假
								flowid = 26;
		...
		return "/dailyWork/ChangeHolidayTaskList";
	}
	
	//调休审批后提交
	@RequestMapping(value = "/over")
	@SystemControllerLog(description = "调休审批后提交")
	public String updata(Model model, Holiday applymaterial) {
		...
		return "redirect:/dailyWork/ChangeHolidayTask/tasklist";
	}
