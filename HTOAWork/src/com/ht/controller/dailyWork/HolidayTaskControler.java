 ...
 */
@Controller
@RequestMapping("/dailyWork/HolidayTask")
public class HolidayTaskControler {
	...
	@SystemControllerLog(description = "进入请假审批页面")
	public String List(Model model) {
					...
					for (int i = 0; i < apdotlist.size(); i++) {
						...
						if (apdot.getFlowid() == fsch.getFlowdot()) {
							...
							if(apdot.getFlowid() == 21){
								...
								// 未完成任务
								if (holidayservice.selectHoliday(applymaterial).size() > 0) {
									materiallist.add(holidayservice.selectHoliday(applymaterial).get(0));
								}
								// 已完成任务
								applymaterial.setApprovalstatus(2);
								overtasklist.addAll(holidayservice.selectHoliday(applymaterial));
								applymaterial.setApprovalstatus(3);
								overtasklist.addAll(holidayservice.selectHoliday(applymaterial));
								break;
							}else if(apdot.getFlowid() ==22){// 若审批节点为 ---财务部审批员工请假
								flowid = 22;
								applymaterial.setApprovalstatus(3);
								String anum = apmlist.get(j).getApplynum();
								applymaterial.setAppnum(anum);
		...
		return "/dailyWork/HolidayTaskList";
	}
	
	//请假审批后提交
	@RequestMapping(value = "/over")
	@SystemControllerLog(description = "请假审批后提交")
